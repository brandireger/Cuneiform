...
Usage:
    python 19_pretrain.py [--config pretrain_config.json] [--tag base] [--resume]
...
def sample_boundary_batch(pool, tok, cfg, rng, device):
    ...
    while len(contexts) < cfg["boundary_batch_size"] and attempts < cfg["boundary_batch_size"] * 20:
        ...
        cross_genre_pool = [(o["fragment_id"], o["ids"]) for o in by_genre.get(ex["genre_band"], [])
                            if o["cth"] != ex["cth"]]
        random_pool = [(o["fragment_id"], o["ids"]) for o in pool if o["cth"] != ex["cth"]]
        neg_pools = {"cross_genre": cross_genre_pool, "random": random_pool}
        result = build_boundary_example(ex["ids"], bp, boundaries, rng, neg_pools,
                                         window=cfg["boundary_window"])
        if result is None:
            continue
        ctx, cont, label = result
        contexts.append(ctx)
        conts.append(cont)
        labels.append(label)
    ...
    return input_ids, boundary_positions, labels_t
...
def main():
    ...
    ap.add_argument("--config", default="pretrain_config.json")
