        ...
        for ev, payload in iter_doc_order(text_child):
            if ev == "start":
                ...
                if tag == "lb":
                    ...
                    #
                    # REJECTED HYPOTHESIS (tested 2026-07-21, DM0 follow-up):
                    # tried "a <space> produces one `cu` damage-marker glyph
                    # iff it falls inside an active <del_in>/<laes_in> span"
                    # -- motivated by tracing ONE orphan-marker example
                    # (KBo 42.91 line 1) by hand, where this held exactly.
                    # Implemented and measured on the FULL corpus: alignment
                    # against `cu` got WORSE, not better (57.87% -> 47.94%
                    # of lines), because the rule overshoots -- most
                    # in-damage-span <space> elements do NOT get a `cu`
                    # glyph (68,970 lines ended up with one extra phantom
                    # token vs. only 6,144 before the change). The single
                    # traced example was not representative; the real rule
                    # governing when a blank run gets a `cu` placeholder is
                    # still unknown. Reverted rather than ship a change that
                    # regresses the metric it was meant to fix -- see
                    # dm0_cu_report.md for the full measured comparison.
                    # decompose_corpus.py's orphan-marker gap (a damage
                    # marker outside any <w> span) remains UNFIXED.
                    pass
            elif ev == "end":
                tag = local(payload.tag)
