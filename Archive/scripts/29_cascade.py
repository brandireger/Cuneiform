#!/usr/bin/env python3
"""
29_cascade.py -- P5 D19: combination & gates per specs/P5_RERANK_SPEC.md.

Usage:
    python scripts/29_cascade.py

Features per (query, candidate) pair: BM25 whole-fragment score,
edge-window score (D16b, N=3 -- the promoted value), boundary-head
seam score (D17) + argmax-offset agreement count (D17b addendum),
edge-continuation lift (D18, H=5).

Combiner: ONE simple monotone combiner, pre-committed -- logistic
regression over the 5 features, trained on TRAIN-side real joins
(916 pairs) + seam-local synthetic pairs (fracture_engine, mode='cut',
split='train') as positives, BM25-mined hard negatives (non-partners)
as negatives. NEVER fit on dev.

DESIGN NOTE: train-side feature computation reuses the exact same
seam-construction code as D17/D18 (2-direction x 4-offset search,
`<MASK>`-span lift at H=5) so train and dev features are on identical
scales -- required for a valid logistic regression. Synthetic pairs'
`member_a_tokens`/`member_b_tokens` (already <LINE>-delimited) are
reshaped into the same (line_idx, tokens)-list structure
`get_fragment_tokens()` returns for real fragments, so the identical
seam-scoring function handles both without a fork.

Ablation grid rows: BM25-alone (baseline, restated) / +D17 / +D18 /
full cascade -- over the D16b-promoted union candidate set (already
the only candidate set generated, since D16b was promoted).

Pre-registered gates G1-G4 at full_distractor scale, dev.
"""
import json
import math
import random
...
def tokens_to_lines(flat_tokens):
    """Reshape a flat <LINE>-delimited token list (synthetic pair
    member) into the same [(idx, [tokens]), ...] structure
    get_fragment_tokens() returns for real fragments."""
    lines, cur = [], []
    idx = 0
...
class FixedVocabBM25:
    """Precomputes IDF/avgdl ONCE from a reference corpus (train_toks),
    then scores arbitrary NEW (query_tokens, doc_tokens) pairs against
    that FIXED vocabulary -- needed because eh.bm25_score_matrix fits a
    fresh vectorizer per call, which is meaningless IDF for a 2-document
    ad-hoc "corpus" (every term's df is 0 or 1). Unseen terms get
    idf=0 (standard BM25 generalization to out-of-vocabulary terms)."""
...
class SeamFeaturizer:
    """Wraps the frozen D14 encoder to compute (seam_score, n_agree,
    d18_lift_H5) for arbitrary (lead_lines, trail_lines) pairs -- the
    SAME computation D17/D18 ran for dev, reused here for train-side
    positives/negatives so features are on an identical scale."""
    ...
    def score_pair(self, q_lines, c_lines):
        """Returns (seam_score, n_agree_over_half, d18_lift_H5,
        best_direction, best_offset)."""
        best = None
        all_flags = []
...
def main():
    ...
    print("Building BM25 whole-fragment + edge-window score lookups (dev candidate pairs)...")
    edge_toks_all = build_edge_window_tokens(frags, line_index, edge_info, EDGE_N)
    all_cand_ids_union = sorted(set(c for lst in candidates.values() for c in lst))
    cand_toks_whole = [json.loads(frags_lookup.loc[c, "sign_attested"]) if c in frags_lookup.index else []
                       for c in all_cand_ids_union]
    ...
    cand_idx = {c: i for i, c in enumerate(all_cand_ids_union)}
    q_idx = {q: i for i, q in enumerate(all_qids_for_features)}

    cand_toks_edge = [edge_toks_all.get(c, []) for c in all_cand_ids_union]
    ...
    print("Building TRAIN-side hard negatives (BM25-mined)...")
    ...
    # Fit IDF/avgdl over the SAME full non-test candidate universe dev_features
    # uses (not just train_toks) -- otherwise train and dev BM25 features would
    # sit on different scales (different corpus sizes -> different IDF
    # magnitudes) and the combiner's learned weight wouldn't transfer correctly.
    print("Fitting fixed-vocabulary BM25 scorers (whole-fragment + edge-window) "
         "over the SAME full non-test universe dev uses...")
    ...
    for i, p in enumerate(synth_pairs):
        ...
        bm25_w = bm25_whole_scorer.score(a_toks, b_toks)
        # edge-window not meaningful for synthetic sub-fragment cuts (no fixed
        # top/bottom N-line geometry on a piece that's already an edge) --
        # approximated as the whole-fragment score itself, documented as a
        # scope simplification in p5_report.md.
    ...
    X = np.array(X)
    y = np.array(y)
    with open(OUT / "p5_train_features.json", "w", encoding="utf-8") as f:
    ...
    def plus_d17(qid, cid):
        ...
        return f[0] + f[2]  # bm25_whole + seam_score (unweighted, simple additive ablation)

    def plus_d18(qid, cid):
        f = dev_features(qid, cid)
        return f[0] + f[4]  # bm25_whole + d18_lift
    ...
    with open(OUT / "p5_ablation_grid.json", "w", encoding="utf-8") as f:
    ...
    g4_pass = dup_casc_r1 >= dup_bm25_r1 - 0.02  # small CI-informed tolerance, documented
    ...
    with open(OUT / "p5_gates.json", "w", encoding="utf-8") as f:
        json.dump(gates, f, ensure_ascii=False, indent=2, default=str)

    print("Gates:", json.dumps(gates, indent=2, default=str)[:3000])
    print("D19 done. p5_ablation_grid.json, p5_gates.json, p5_train_features.json written.")
